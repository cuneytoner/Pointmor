...
- **Bootstrap Split / AI Compliance Operations Endpoint (Step 13):** **COMPLETE (Authoritative)** - `GET /admin/products/ai-compliance/operations` endpoint birincil veri kaynağı oldu. Platform admin cross-organization görünürlüğü destekliyor. Shared backend read model (`loadAiComplianceOperationsForScope`) kullanıyor. `/admin/bootstrap` `moduleOperations.aiCompliance.systems` KALDIRILDI (Step 14 ile). Frontend `AiComplianceOperationsPage` yeni hook'u kullanıyor. API testleri eklendi.
- **Bootstrap Cleanup & Contract Hardening (Step 14):** **COMPLETE** - Bootstrap contract son haline getirildi. `moduleOperations.aiCompliance.systems` alanı tamamen kaldırıldı. Bootstrap artık sadece sayısal metrikler döndürüyor, operasyonel sistem listesi değil. Frontend tipleri hardened edildi, TypeScript gelecekteki regressions'ları yakalar. `/admin/bootstrap` sadece minimal cross-product veri döndürür. AI Compliance operasyonel sistemleri `GET /admin/products/ai-compliance/operations` endpoint'ine aittir. Bootstrap contract testi eklendi.
- **Persisted Operational Events Foundation (Step 15):** **COMPLETE** - AI Compliance için kalıcı operasyonel olaylar temeli eklendi. `AiOperationalEvent` modeli ile append-only olay kaydı sistemi kuruldu. `recordAiOperationalEvent` helper fonksiyonu ile tenant-scope doğrulamalı olay kaydı ve actor membership validasyonu. Sistem oluşturma, değerlendirme gönderme, obligation/task oluşturma akışlarına olay kaydı entikre edildi. `GET /admin/products/ai-compliance/operations` endpoint'ine operational events eklendi, metadata sanitization ile. Frontend timeline render'ı persisted olayları önceliklendiriyor, dedupe stratejisi ile duplicate'leri engelliyor. Seed verilerine sentetik operational events eklendi. Çapraz tenant olay sızıntısını önleyen ve metadata sanitization testleri eklendi.
- **AI Compliance Workflow Actions Foundation (Step 17):** **COMPLETE** - Minimal operasyonel workflow aksiyonları eklendi: task complete, obligation review, assessment reopen ve manuel reviewer assignment. Aksiyonlar `POST /admin/products/ai-compliance/...` altında authenticated endpoint'lerle çalışır; tenant scope, platform admin cross-org davranışı, `ai_act` module activation ve mevcut izin modeli korunur. Her başarılı aksiyon kalıcı `AiOperationalEvent` üretir ve operational/system timeline içinde sanitized label ile görünür. `/admin/bootstrap` hâlâ operasyonel sistem/event taşımaz; notification/SLA/automation engine kapsam dışı ve backlog olarak kalır.
