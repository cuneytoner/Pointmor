...
| **`npm run db:seed:full:demo`** (`apps/api`) | [`prisma/seed-full-demo.ts`](../apps/api/prisma/seed-full-demo.ts) + [`prisma/seed.ts`](../apps/api/prisma/seed.ts) | Guard’lı full seed: `APP_ENV=demo`, `ALLOW_FULL_DEMO_SEED=true`, `CONFIRM_FULL_DEMO_SEED=I_UNDERSTAND_FULL_DEMO_SEED` ve `DATABASE_URL` içinde `pointmor_demo` (veya `FULL_DEMO_SEED_DB_URL_SUBSTR`) doğrulaması. İçeride `SEED_FULL_DEMO=1` set eder; opsiyonel `FORCE_RESEED_DEMO=1` ile senaryo verisini sıfırlar. |
