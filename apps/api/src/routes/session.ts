...
import { loadAiComplianceCountsForScope } from "../lib/ai-compliance-operations.js";
...
async function loadModuleOperationsForSession(
  ...
  // Build scope for AI Compliance counts (minimal, no systems)
  const scope = s.user.platformAdmin
    ? { mode: "platform_admin" as const }
    : scopedTenantIds.length > 0
      ? { mode: "cross_org" as const, tenantIds: scopedTenantIds }
      : { mode: "tenant" as const, tenantId: "__none__" };

  // Load minimal AI Compliance counts (no systems) using shared function
  const aiComplianceCounts = await loadAiComplianceCountsForScope(scope);

  // Load other module operations (loyalty, advisor) directly
